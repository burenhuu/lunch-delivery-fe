export const cartAnimation = (event: any) => {
    ...
    let imgToDrag = getClosest(event.target, ".product-cart");
    let viewCart = document.getElementsByClassName("my-cart")[0];
    ...
    image.style =
        "z-index: 11111; width: 120px; opacity:1; position:fixed; top:" +
        disTop +
        "px;left:" +
        disLeft +
        "px; transition: left 1s, top 1s, width 1s, opacity 1s cubic-bezier(1, 1, 1, 1); border-radius: 120px; overflow: hidden; box-shadow: 0 21px 36px rgba(0,0,0,0.1)";
